...
public class InvertedIndex {
 ...
 /**
  * Variables Used for pseudoRelevance feedback
   */
  // Whether or not to use pseudo feedback
  public boolean pseudofeedback = false;
  // num of first docs to set as good
  public static int pseudoCheck = 0;
  
  // Parameters
  public static double feedbackALPHA = 0;
  public static double feedbackBETA = 0;
  public static double feedbackGAMMA = 0;
  ...
  // Second constructor of InvertedIndex that also has pseudofeedback variables
  public InvertedIndex(File dirFile, short docType, boolean stem, boolean feedback, boolean pseudofeedback, boolean feedbackparams,
		double feedbackALPHA, double feedbackBETA, double feedbackGAMMA, int pseudoCheck) {
    this.dirFile = dirFile;
    this.docType = docType;
    this.stem = stem;
    this.feedback = feedback;
	this.pseudofeedback = pseudofeedback;
	this.pseudoCheck = pseudoCheck;
  ...
  public Retrieval[] retrieve(Document doc) {
	...
	Retrieval[] retrievals = retrieve(vector);
	  
	  // If pseudofeedback, apply it to the retrieval
	  if (pseudofeedback) {
		Feedback fdback = null;
        fdback = new Feedback(vector, retrievals, this);
		// Run pseudo method in feedback.java
		if (!feedbackparams)
			fdback.pseudoFeedback(pseudoCheck);
		else {
			fdback.pseudoFeedback(pseudoCheck, feedbackALPHA, feedbackBETA, feedbackGAMMA);
		}
		// Set new retrievals based on pseudo method
		retrievals = retrieve(fdback.newQuery());
	}
  ...
  public Retrieval[] retrieve(HashMapVector vector) {
    ...
    for (Map.Entry<String, Weight> entry : vector.entrySet()) {
      ...
      // Determine the scoreadded to the similarity of each document
  ...
  public static void main(String[] args) {
    ...
    boolean stem = false, feedback = false, pseudofeedback = false, feedbackparams = false;
    for (int i = 0; i < args.length - 1; i++) {
      ...
      if (flag.equals("-html"))
        ...
        feedback = true;
		// checks whether to do pseudofeedback
	  else if (flag.equals("-pseudofeedback")) {
        // Use relevance feedback
        pseudofeedback = true;
		// If so, get pseudo int number
		pseudoCheck = Integer.parseInt(args[i + 1]);   
		i++;
	  }
	  // check if we got params with pseudo feedback
	  else if (flag.equals("-feedbackparams")) {
        // Use relevance feedback
		feedbackparams = true;
		feedbackALPHA = Double.parseDouble(args[i + 1]);  
		feedbackBETA = Double.parseDouble(args[i + 2]);  
		feedbackGAMMA = Double.parseDouble(args[i + 3]);  
		i = i + 3;
	  }
      else {
