...
public class ExperimentRated extends Experiment {
  ...
  // Second constructor if pseudofeedback is desired
  public ExperimentRated(File corpusDir, File queryFile, File outFile, short docType, boolean stem, boolean pseudofeedback, boolean feedbackparams)
      throws IOException {
      super(corpusDir, queryFile, outFile, docType, stem, pseudofeedback, feedbackparams);
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < args.length - 3; i++) {
      ...
      if (flag.equals("-html"))
        ...
        // Checks if parameters for pseudofeedback
		feedbackparams = true;
	...
	// Depending on if pseudofeedback is desired, call the correct constructor
	ExperimentRated exper = null;
	if (pseudofeedback){
		exper = new ExperimentRated(new File(corpusDir), new File(queryFile),
			new File(outFile), docType, stem, pseudofeedback, feedbackparams);
	}
	else {
		exper = new ExperimentRated(new File(corpusDir), new File(queryFile),
